...
append_ports()
{
  if get_user_yn "Do you want to append other $1 ports to the existing settings (y,N)?" "n"; then
...
setup_conf_file()
{
  ...
  if get_user_yn "Do you have an internal (LAN) interface that you want to setup (y/N)?" "n"; then
    ...
    # Manage settings for the internal interface(s) if no one is entered.
